...
import re
import numpy as np
from src.tasks.task_abc import Question, Document, Task
from src.utils_tools.io_utils import jload_list, jload
from src.utils_tools.prompt_utils import (format_name, uncapitalize_first, second_last_character,
...
class QuALITYQuestion(Question):
    ...
    def prompt(
            ...
            add_full_document_context: bool=False
    ):
        if add_full_document_context:
            formatted = document_context + "\n\n"+ "### Question\n"
    ...
    def llama_parse_answer(self, raw_output: str):
        ...
        if raw_output is None:
            ...
            match = re.search(r'【ANSWER】\s*([ABCD])', raw_output)
            if match:
...
class QuALITYArticle(Document):
    ...
    def full_content(self):
        """ Full article content """
        result = f"Below is an article titled \"{self.title}\", written by {format_name(self.author)} in {self.year}.\n"
        result += f"{self.text}"
    ...
    def question_prompts(self, add_document_context: bool, add_thought_process: bool, sep_after_question: str, add_full_document_context: bool=False):
        ...
        for q in self.questions:
            prompts.append(
                q.prompt(
                    self.full_content if add_full_document_context else
                    (self._article_context if add_document_context else None),
                    add_thought_process,
                    sep_after_question,
                    add_full_document_context
                )
            )
...
class QuALITY(Task):
    ...
    @staticmethod
    def _load_split(split: str, base_path: str = 'QuALITY'):
        file_path = f'{base_path}/QuALITY.v1.0.1.htmlstripped.{split}'
    ...
    def __init__(self, split: str, base_path: str = 'QuALITY'):
        self.split = split
        if split in ['train', 'dev']:
            self._data = QuALITY._load_split(split, base_path)
            self._create_article()
        elif split in ['all', '50']:
            self._data = QuALITY._load_split('train', base_path) + QuALITY._load_split('dev', base_path)
    ...
    def all_questions(self, add_document_context: bool, add_thought_process: bool, sep_after_question: str, add_full_document_context: bool=False):
        prompts = []
        for document in self.documents:
            prompts += document.question_prompts(add_document_context, add_thought_process, sep_after_question, add_full_document_context)
